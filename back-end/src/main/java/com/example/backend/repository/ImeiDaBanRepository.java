...
public interface ImeiDaBanRepository extends JpaRepository<ImeiDaBan, Long> {
    ...
    @Query(value = " select i.id as 'idImeiDaBan', i.code_imei as 'codeImeiDaBan'\n" +
            "             from imei_da_ban i join bill_detail b on i.id_bill_detail = b.id join sku s on b.id_sku = s.id\n" +
            "             where b.id = ?1 and s.id =?2 and (i.code_imei like %?3%) and i.status = 2", nativeQuery = true)
    ...
    @Modifying
    @Transactional
    @Query(value = "delete from imei_da_ban where id_bill_detail in (select id from bill_detail where id_bill = ?1)", nativeQuery = true)
    void returnStatusImeiWhereIdBill(int id);
    ...
    @Transactional
    @Query(value = "delete from imei_da_ban where id_bill_detail in (select id from bill_detail where id_bill = ?1)", nativeQuery = true)
    void deleteImeiDaBanByIdBill(int id);
}
