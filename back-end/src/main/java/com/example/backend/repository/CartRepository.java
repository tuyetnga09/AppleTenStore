...
public interface CartRepository extends JpaRepository<Cart, Integer> {
    ...
    @Query(value = "select cd.id AS 'ID Cart Detail', p.id AS 'Id Sku', p2.name AS 'Name Product', p.capacity AS 'Capacity', p.color AS 'Color',cd.date_create AS 'DATECREATE', cd.price AS 'Price', cd.quantity AS 'Quantity',  cd.id AS 'Id cart' , p.price * cd.quantity AS 'Total' from cart c join account a on c.id_account = a.id join cart_detail cd on c.id = cd.id_cart\n" +
